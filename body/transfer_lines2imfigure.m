function imdata = transfer_lines2imfigure( lines_pts, fiber_dis )
    ...
    for linenum = 1:n-1
        ...
        if kx1 == kx2   
            if ky2 > ky1
                if abs(y_fiber(ky2) - py2) < 1e-6
                    imdata(kx1:kx1+1, ky1:ky2) = true;
                else
                    imdata(kx1:kx1+1, ky1:ky2+1) = true;
                end
            else
                if abs(y_fiber(ky1) - py1) < 1e-6
                    imdata(kx1:kx1+1, ky2:ky1) = true;
                else
                    imdata(kx1:kx1+1, ky2:ky1+1) = true;
                end
            end
        elseif ky1 == ky2   
            if abs(x_fiber(kx2) - px2) < 1e-6
                imdata(kx1:kx2, ky1:ky1+1) = true;
            else
                imdata(kx1:kx2+1, ky1:ky1+1) = true;
            end
        elseif (ky2 > ky1) && (ky2 - ky1) <= (kx2 - kx1)    %% 线段与X轴的夹角 0-45度
             ...
             for i = ky1+1 : ky2
                ...
                beginy = i - 1;
                endy = i;
                imdata(beginx:endx, beginy:endy) = true;
                beginx = findkx;    
                if i== ky2 && abs(y_fiber(ky2) - py2) > 1e-6
                    if abs(x_fiber(kx2) - px2) > 1e-6
                        imdata(beginx:kx2+1, endy:endy + 1) = true;
                    else
                        imdata(beginx:kx2, endy:endy + 1) = true;
                    end
                end 
             end  
        elseif (ky2 > ky1) && (ky2 - ky1) >= (kx2 - kx1)    %% 线段与X轴的夹角 45-90度
            ...
            for i = kx1 + 1: kx2
                ...
                if abs(y_fiber(findky) - ky) > 1e-6
                    imdata(beginx:endx, beginy:findky + 1) = true;
                else
                    imdata(beginx:endx, beginy:findky) = true;
                end
                beginy = findky;
                %需要把结尾补全
                if i == kx2 && abs(x_fiber(kx2) - px2) > 1e-6
                    if abs(y_fiber(ky2) - py2) > 1e-6
                        imdata(kx2:kx2+1, beginy:ky2 + 1) = true;
                    else
                        imdata(kx2:kx2+1, beginy:ky2) = true;
                    end
                end  
            end
        elseif (ky2 < ky1) && (ky1 - ky2) <= (kx2 - kx1)    %% 线段与X轴的夹角 0-负45度
             ...
             for i = ky2+1 : ky1
                ...
                if i== ky1 && abs(y_fiber(ky1) - py1) > 1e-6
                    imdata(kx1: beginx + 1, endy:endy+1) = true;
                end
             end    
            ...
            for i = kx1 + 1: kx2
                ...
                if i == kx2 && abs(x_fiber(kx2) - px2) > 1e-6
                    if abs(y_fiber(ky2) - py2) > 1e-6
                        imdata(kx2: kx2+1, ky2:endy) = true;
                    else
                        imdata(kx2: kx2+1, ky2+1:endy) = true;
                    end
                end  
            end
        end
